...
class ActiveSuspensionController(Node):
    ...
    def __init__(self):
        ...
        self.create_subscription(Imu, '/imu', self.imu_cb, 10)  # FIX 1: was never subscribed
    ...
    def imu_cb(self, msg):
        ...
        if self.last_imu_time is not None:
            ...
            if dt > 0.1:   # ignore huge gaps at startup
            ...
            # IMU path — integrate
            imu_vel = self.yc_vel + acc_z * dt
            imu_pos = self.yc_pos + imu_vel * dt

            # Complementary filter — anchor low frequency to joint encoder
            # Use average of L and R joint positions as the body position reference
            joint_anchor = (self.state_L[2] + self.state_R[2]) / 2.0

            self.yc_vel = self.ALPHA * imu_vel + (1 - self.ALPHA) * 0.0
            self.yc_pos = self.ALPHA * imu_pos + (1 - self.ALPHA) * joint_anchor
    ...
    def joint_cb(self, msg):
        ...
        for i, name in enumerate(msg.name):
            if name == 'suspension_L':
                pos = msg.position[i]   # FIX 3: was referenced as joint_pos (NameError)
                vel = msg.velocity[i]   # FIX 3: was referenced as joint_vel (NameError)

                self.state_L = np.array([
                    self.yc_pos,                    # yc  — chassis body (from IMU)
                    self.yc_vel,                    # ẏc  — chassis body (from IMU)
                    self.nominal_height + pos,      # yk  — foot link (from joint encoder)
                    vel                             # ẏk  — foot link (from joint encoder)
                ])

            elif name == 'suspension_R':
                pos = msg.position[i]   # FIX 4: state_R was still using old mirroring
                vel = msg.velocity[i]

                self.state_R = np.array([
                    self.yc_pos,                    # yc  — same chassis IMU (rigid body)
                    self.yc_vel,                    # ẏc  — same chassis IMU (rigid body)
                    self.nominal_height + pos,      # yk  — right foot (from joint encoder)
                    vel                             # ẏk  — right foot (from joint encoder)
                ])
